...
def generate_ome_meta_for_mask(
...
def remove_labels(
    ...
    val_to_remove = [
        val for val in val_to_remove if val not in exclude_from_val_to_remove
    ]
...
def remove_overlapping_labels(
    img: Image, overlap: int, mode: str
) -> Tuple[Image, List[int]]:
...
def find_and_remove_overlapping_labels_in_first_channel(
    ...
    for i in range(0, y_ntiles):
        for j in range(0, x_ntiles):
            ...
            task.append(
                dask.delayed(remove_overlapping_labels)(
                    tiles[n], overlap, label_remove_mode
                )
            )
            n += 1
    ...
    for i in range(0, y_ntiles):
        for j in range(0, x_ntiles):
            ...
            if top_left_tile_id is not None:
                tile, border_map = replace_overlapping_border_labels(
                    modified_tiles[top_left_tile_id],
                    tile,
                    overlap,
                    "horizontal+vertical",
                )
...
def replace_overlapping_border_labels_in_another_channel(
    ...
    for i, tile in enumerate(tiles):
        task.append(
            dask.delayed(replace_values)(tile, border_maps[i], tile_additions[i], dtype)
        )
    modified_tiles = dask.compute(*task)
    return list(modified_tiles)
...
def modify_tiles_first_channel(
    ...
    List[Image], Dict[int, Dict[int, int]], Dict[int, Dict[int, int]], List[int]
]:
...
def process_all_masks(
    ...
    if len(tiles[0].shape) > 2 and tiles[0].shape[0] > 1:
        tiles_nuc = [t[0, :, :] for t in tiles]
        tiles_cell = [t[1, :, :] for t in tiles]
        raw_tile_groups = [tiles_cell, tiles_nuc]
    else:
        if len(tiles[0].shape) > 2:
            tiles = [t.squeeze() for t in tiles]
        tiles_nuc = tiles
        raw_tile_groups = [tiles]
    ...
    if no_cell == False:
        ...
        tile_additions = tile_additions_cell

    if no_cell:
        all_exclusions = excluded_labels_nuc
        all_border_maps = border_maps_nuc
        tile_additions = tile_additions_nuc

    mod_tile_groups = []
    for tile_group in raw_tile_groups:
        mod_tile_group = modify_tiles_another_channel(
            tile_group, all_exclusions, all_border_maps, tile_additions, dtype
        )
    ...
    if no_cell:
        matched_masks = stitched_imgs
        fraction_matched = 1.0
    else:
        matched_masks, fraction_matched = get_matched_masks(
            cell_mask=stitched_imgs[1],
            nucleus_mask=stitched_imgs[0],
            dtype=dtype,
